...
type EventQuery struct {
	...
	withType         *OrganizationEventTypeQuery
...
func (eq *EventQuery) QueryTransactions() *TransactionQuery {
	query := (&TransactionClient{config: eq.config}).Query()
...
func (eq *EventQuery) QueryType() *OrganizationEventTypeQuery {
	query := (&OrganizationEventTypeClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EventQuery) WithTransactions(opts ...func(*TransactionQuery)) *EventQuery {
	query := (&TransactionClient{config: eq.config}).Query()
...
func (eq *EventQuery) WithType(opts ...func(*OrganizationEventTypeQuery)) *EventQuery {
	query := (&OrganizationEventTypeClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if query := eq.withTransactions; query != nil {
		if err := eq.loadTransactions(ctx, query, nodes,
			func(n *Event) { n.Edges.Transactions = []*Transaction{} },
			func(n *Event, e *Transaction) { n.Edges.Transactions = append(n.Edges.Transactions, e) }); err != nil {
			return nil, err
		}
	}
	if query := eq.withType; query != nil {
		if err := eq.loadType(ctx, query, nodes, nil,
			func(n *Event, e *OrganizationEventType) { n.Edges.Type = e }); err != nil {
			return nil, err
		}
	}
...
func (eq *EventQuery) loadTransactions(ctx context.Context, query *TransactionQuery, nodes []*Event, init func(*Event), assign func(*Event, *Transaction)) error {
	...
	query.Where(predicate.Transaction(func(s *sql.Selector) {
		s.Where(sql.InValues(event.TransactionsColumn, fks...))
	}))
...
func (eq *EventQuery) loadType(ctx context.Context, query *OrganizationEventTypeQuery, nodes []*Event, init func(*Event), assign func(*Event, *OrganizationEventType)) error {
	...
	query.Where(organizationeventtype.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
