...
type OrganizationQuery struct {
	...
	withUsers        *UserQuery
	withAccountTypes *OrganizationAccountTypeQuery
	withEventTypes   *OrganizationEventTypeQuery
...
// QueryAccountTypes chains the current query on the "accountTypes" edge.
func (oq *OrganizationQuery) QueryAccountTypes() *OrganizationAccountTypeQuery {
	query := (&OrganizationAccountTypeClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryEventTypes chains the current query on the "eventTypes" edge.
func (oq *OrganizationQuery) QueryEventTypes() *OrganizationEventTypeQuery {
	query := (&OrganizationEventTypeClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) Clone() *OrganizationQuery {
	...
	return &OrganizationQuery{
		...
		withAccountTypes: oq.withAccountTypes.Clone(),
		withEventTypes:   oq.withEventTypes.Clone(),
...
// WithAccountTypes tells the query-builder to eager-load the nodes that are connected to
// the "accountTypes" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrganizationQuery) WithAccountTypes(opts ...func(*OrganizationAccountTypeQuery)) *OrganizationQuery {
	query := (&OrganizationAccountTypeClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withAccountTypes = query
	return oq
}

// WithEventTypes tells the query-builder to eager-load the nodes that are connected to
// the "eventTypes" edge. The optional arguments are used to configure the query builder of the edge.
func (oq *OrganizationQuery) WithEventTypes(opts ...func(*OrganizationEventTypeQuery)) *OrganizationQuery {
	query := (&OrganizationEventTypeClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withEventTypes = query
	return oq
}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	var (
		...
		loadedTypes = [4]bool{
			oq.withUsers != nil,
			oq.withAccountTypes != nil,
			oq.withEventTypes != nil,
			oq.withEvents != nil,
		}
	)
	...
	if query := oq.withAccountTypes; query != nil {
		if err := oq.loadAccountTypes(ctx, query, nodes,
			func(n *Organization) { n.Edges.AccountTypes = []*OrganizationAccountType{} },
			func(n *Organization, e *OrganizationAccountType) {
				n.Edges.AccountTypes = append(n.Edges.AccountTypes, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := oq.withEventTypes; query != nil {
		if err := oq.loadEventTypes(ctx, query, nodes,
			func(n *Organization) { n.Edges.EventTypes = []*OrganizationEventType{} },
			func(n *Organization, e *OrganizationEventType) { n.Edges.EventTypes = append(n.Edges.EventTypes, e) }); err != nil {
			return nil, err
		}
	}
...
func (oq *OrganizationQuery) loadAccountTypes(ctx context.Context, query *OrganizationAccountTypeQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *OrganizationAccountType)) error {
	...
	query.Where(predicate.OrganizationAccountType(func(s *sql.Selector) {
		s.Where(sql.InValues(organization.AccountTypesColumn, fks...))
	}))
...
func (oq *OrganizationQuery) loadEventTypes(ctx context.Context, query *OrganizationEventTypeQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *OrganizationEventType)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.OrganizationEventType(func(s *sql.Selector) {
		s.Where(sql.InValues(organization.EventTypesColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (oq *OrganizationQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Event)) error {
	...
	query.Where(predicate.Event(func(s *sql.Selector) {
		s.Where(sql.InValues(organization.EventsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
