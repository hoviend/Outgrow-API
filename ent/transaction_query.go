...
func (tq *TransactionQuery) QueryAccount() *OrganizationAccountQuery {
	query := (&OrganizationAccountClient{config: tq.config}).Query()
...
func (tq *TransactionQuery) QueryEvent() *EventQuery {
...
func (tq *TransactionQuery) WithAccount(opts ...func(*OrganizationAccountQuery)) *TransactionQuery {
	query := (&OrganizationAccountClient{config: tq.config}).Query()
...
func (tq *TransactionQuery) WithEvent(opts ...func(*EventQuery)) *TransactionQuery {
	query := (&EventClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(transaction.FieldEventID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Transaction.Query().
//		Select(transaction.FieldEventID).
//		Scan(ctx, &v)
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := tq.withAccount; query != nil {
		if err := tq.loadAccount(ctx, query, nodes, nil,
			func(n *Transaction, e *OrganizationAccount) { n.Edges.Account = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withEvent; query != nil {
		if err := tq.loadEvent(ctx, query, nodes, nil,
			func(n *Transaction, e *Event) { n.Edges.Event = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TransactionQuery) loadAccount(ctx context.Context, query *OrganizationAccountQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *OrganizationAccount)) error {
	...
	query.Where(organizationaccount.IDIn(ids...))
...
func (tq *TransactionQuery) loadEvent(ctx context.Context, query *EventQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Event)) error {
